...
const UploadCourse = () => {
  ...
  const uploadHandler = (e) => {
    e.preventDefault()
    console.log(e.target.title.value,'tt')
    if(e.target){
        if(e.target.title.value && e.target.description.value && e.target.video.files.length){
          if(token){
            let formData = new FormData()
            formData.append('title',e.target.title.value)
            formData.append('description',e.target.description.value)
            formData.append('coverPhoto',e.target.image.files[0])
    ...
    <div className='home' id="upload-page">
       ...
       <motion.form 
        ...
        >
          ...
          <SecondStep setStep={setStep} setError={setError} />  
       </motion.form>
